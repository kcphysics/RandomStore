<script>
    ...
    function updateItem(event) {
        ...
        var oshop = get(shop);
        var existing_ndx = oshop.items.findIndex(
            (item) => item.name === itemName
        );
        oshop.items[existing_ndx] = Object.assign(
            oshop.items[existing_ndx],
            newItem
        );
        shop.set(oshop);
    }
    ...
    {#if $shop.items.length > 0}
        ...
        {#if !$isViewingShop}
            <div class="listControls">
                <button on:click={showSaveDialog}
                    >Save for others to use <Icon data={save} /></button
                >
                <button on:click={showAddNewDialog}
                    >Add New Item <Icon data={plus} /></button
                >
            </div>
        {/if}
        ...
        <table class="ShopListTable">
            <tr>
                ...
                {#if !$isViewingShop}<th class="controls" />{/if}
            </tr>
    ...
    {#if !$isViewingShop}
        <dialog bind:this={confirmSaveDlg}>
            <form id="confirmSave">
                ...
                <p>
                    However, to keep it free, if no one has viewed or used this
                    store in 30 days, it will be deleted
                </p>
                <button id="saveShopBtn" on:click|preventDefault={saveStore}
                    >Save</button
                >
                <button id="cancelSaveBtn" on:click|preventDefault={cancelSave}
                    >Cancel</button
                >
            </form>
        </dialog>
        <dialog bind:this={addNewItemDlg}>
            <form>
                ...
                />
                <button class="saveBtn" on:click|preventDefault={addNewItem}
                    >Save</button
                >
                <button class="cancelBtn" on:click|preventDefault={cancelAddNew}
                    >Cancel</button
                >
            </form>
        </dialog>
        <dialog bind:this={urlDisplayDlg}>
            ...
            <div class="urlText">
                ...
                <button
                    class="saveBtn"
                    on:click|preventDefault={copyUrlToClipboard}
                    ><Icon data={completeIcon} /></button
                >
            </div>
            <button class="cancelBtn" on:click|preventDefault={closeUrlDlg}
                >Close</button
            >
        </dialog>
        <dialog bind:this={changeStoreNameDlg}>
            <form>
                ...
                <div class="formControl">
                    <button
                        class="saveBtn"
                        on:click|preventDefault={saveNewStoreName}>Save</button
                    >
                    <button
                        class="cancelBtn"
                        on:click|preventDefault={cancelNewStoreName}
                        >Cancel</button
                    >
                </div>
            </form>
